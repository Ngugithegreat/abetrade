...
export async function GET() {
  ...
  const [userRows, txns, openTrades, closedTrades, refStats] = await Promise.all([
    sql`SELECT id, name, email, role, balance, demo_balance, country, phone, status, kyc_status, kyc_reason, bonus_locked, is_test, test_win_pct FROM abetrade_users WHERE id = ${session.id}` as Promise<any[]>,
