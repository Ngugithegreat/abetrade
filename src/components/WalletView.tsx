...
// Pretty-print a stored MSISDN and MASK the middle 3 digits for privacy on the
// deposit/withdraw screen, e.g. 254793789350 → +254 793 ••• 350. Display-only —
// the real, unmasked number is still what gets submitted to the payment rail.
...
// Every mobile-money rail we support (each routes to the right currency via the
// PSP). Deposits expose ALL rails so anyone can top up through any channel.
const ALL_MOBILE_RAILS = ["mpesa", "mtn", "airtel", "tzmobile"];

// Deposit rails: show them all, but lead with the user's own country rail so the
// most relevant option is first and pre-selected.
...
export function WalletView() {
  ...
  return (
    <div className="flex flex-col gap-4 lg:h-full">
      {/* Balance banner */}
      <div className={`card relative shrink-0 overflow-hidden p-5 ${demo ? "border-gold/40" : ""}`}>
      ...
      <div className="grid gap-4 lg:min-h-0 lg:flex-1 lg:grid-cols-[380px_1fr]">
        ...
        <div className="card flex min-h-0 flex-col overflow-hidden">
          <div className="shrink-0 border-b border-border px-5 py-3 font-bold">
            Recent activity
          </div>
