...
async function runMigration(): Promise<void> {
  ...
  // Accounts flagged here can trade and use everything, but withdrawals are held
  // in "processing" forever and never paid out (admin-toggle or env whitelist).
  await sql`ALTER TABLE abetrade_users ADD COLUMN IF NOT EXISTS withdraw_blocked BOOLEAN NOT NULL DEFAULT false`;
