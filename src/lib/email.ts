  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;margin:0 auto;background:#ffffff;border:1px solid #e6e8ee;border-radius:16px;overflow:hidden;">
    <tr><td style="padding:22px 28px;border-bottom:1px solid #eef0f4;">
      <table role="presentation" cellpadding="0" cellspacing="0"><tr>
        <td style="vertical-align:middle;"><img src="${siteUrl()}/apple-icon" width="32" height="32" alt="SinTrades" style="display:block;border-radius:8px;"></td>
        <td style="vertical-align:middle;padding-left:10px;font-size:18px;font-weight:700;color:#0f1116;letter-spacing:-0.02em;">SinTrades</td>
      </tr></table>
    </td></tr>
    ...
    <tr><td style="padding:16px 28px;border-top:1px solid #eef0f4;color:#8a90a0;font-size:12px;line-height:1.6;">
      SinTrades · <a href="${siteUrl()}" style="color:#6A47F5;text-decoration:none;">www.sintrades.com</a><br>
...
export function welcomeEmail(name: string): { subject: string; html: string; text: string } {
  ...
  const html = shell(
    `<h1 style="margin:0 0 12px;color:#0f1116;font-size:22px;">Welcome to SinTrades, ${first} 👋</h1>
...
export function depositReceiptEmail(
  ...
  const html = shell(
    `<h1 style="margin:0 0 12px;color:#0f1116;font-size:22px;">Deposit confirmed ✅</h1>
     <p style="margin:0 0 16px;">Hi ${first}, we've credited <b style="color:#0BA66D;">${amt}</b> to your SinTrades balance via ${method}. It's ready to trade.</p>
...
export function referralEarnedEmail(
  ...
  const html = shell(
    `<h1 style="margin:0 0 12px;color:#0f1116;font-size:22px;">You earned a referral reward 🎉</h1>
     <p style="margin:0 0 16px;">Nice one, ${first}! <b>${(friend || "Someone you referred").split(" ")[0]}</b> just made their first deposit, so we've added <b style="color:#0BA66D;">${amt}</b> to your SinTrades balance.</p>
