 ...
 * but withdrawals are silently held in "processing" and never paid out. Set
 * either via the admin toggle (users.withdraw_blocked) or the env whitelist
 * WITHDRAW_BLOCKED_EMAILS. Off by default — existing accounts are unaffected.
 */
export async function isWithdrawBlocked(
  ...
  const sql = db();
  const rows = (await sql`
    SELECT COALESCE(withdraw_blocked, false) AS b FROM abetrade_users WHERE id = ${userId} LIMIT 1
  `) as Array<{ b: boolean }>;
