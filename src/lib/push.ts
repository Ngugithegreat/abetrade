...
// Optional:                  VAPID_SUBJECT (default mailto:support@sintrades.com)
...
function ensureVapid(): boolean {
  ...
  if (!configured) {
    webpush.setVapidDetails(
      process.env.VAPID_SUBJECT || "mailto:support@sintrades.com",
...
export async function sendPushToUser(userId: number, payload: PushPayload): Promise<void> {
  ...
  try {
    ...
    const subs = (await sql`
      SELECT endpoint, p256dh, auth FROM abetrade_push_subs WHERE user_id = ${userId}
      ...
      subs.map(async (s) => {
        try {
          ...
          if (e?.statusCode === 404 || e?.statusCode === 410) {
            await sql`DELETE FROM abetrade_push_subs WHERE endpoint = ${s.endpoint}`.catch(() => {});
          }
        }
      })
    );
